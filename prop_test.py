...
# --- Base lot size calculation using risk-per-trade ---
...
# --- Single asset backtest executor ---
    ...
    for i in range(len(df)):
        ...
        if position is None:
            if signal in ["BUY", "SELL"]:
                asset_class, symbol = df.loc[i, "asset_class"], df.loc[i, "symbol"]
...
def run_multi_asset_backtest(selected_assets, starting_balance, leverage, lot_multiplier,
    ...
    if not equity_combined.empty:
        ...
        if account_equity_df["equity"].iloc[0] == 0:
            account_equity_df["equity"] += starting_balance
    else:
...
def main():
    ...
    if st.button("🚀 Run Backtest"):
        with st.spinner("Running backtest across assets..."):
