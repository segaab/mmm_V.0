...
# -------------------
# Helper: Convert start/end dates to Yahoo period
# -------------------
def date_to_period(start_date, end_date):
    """Convert start/end dates to YahooQuery compatible period (days)"""
    delta = end_date - start_date
    return delta.days + 1

# -------------------
# Fetch RVol Data (cached)
# -------------------
@st.cache_data(show_spinner=True)
def fetch_rvol_data(symbol, start_date, end_date, rolling_window=5):
    period_days = date_to_period(start_date, end_date)
    t = Ticker(symbol, timeout=60)
    hist = t.history(period=f"{period_days}d", interval="1h")
...
def calculate_rvol(df, window=5):
    if df is None or df.empty:
        return None
    df = df.copy()
    df = df.sort_values("datetime_gmt3")
...
def detect_gap_up(df, session_hours, threshold=2.0):
    ...
    df = df.copy()
    df["date"] = df["datetime_gmt3"].dt.date
    df["hour"] = df["datetime_gmt3"].dt.hour
...
# -------------------
def backtest_asset(symbol, session_hours, threshold, holding_period):
    df = fetch_rvol_data(symbol, start_date, end_date, rolling_window)
    if df is None or df.empty:
        return None, symbol
    trades = []
    dates = sorted(df["datetime_gmt3"].dt.date.unique())
    for i in range(1, len(dates)-holding_period):
        temp_df = df[df["datetime_gmt3"].dt.date <= dates[i]].copy()
        ...
        if gap_ratio is not None and gap_ratio >= threshold:
            entry_date = dates[i]
            entry_price = df[df["datetime_gmt3"].dt.date == entry_date]["close"].iloc[-1]
            exit_date = dates[i + holding_period]
            exit_df = df[df["datetime_gmt3"].dt.date == exit_date]
...
if st.sidebar.button("Run Backtest"):
    if not selected_assets:
        st.warning("Please select at least one asset to run the backtest.")
    else:
