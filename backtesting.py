...
# --- Load Price Data ---
def load_price_data(asset_name: str, years_back: int = 3) -> pd.DataFrame:
    ticker = assets.get(asset_name, asset_name)
    end_date = datetime.datetime.today()
    start_date = end_date - datetime.timedelta(days=years_back*365)
    start_str = start_date.strftime('%Y-%m-%d')
    end_str = end_date.strftime('%Y-%m-%d')

    logger.info(f"Fetching Yahoo price data for {ticker} from {start_str} to {end_str}")
    attempt = 0
    while attempt < 3:
        try:
            t = Ticker(ticker)
            ...
            attempt += 1
            time.sleep(2)
    logger.error(f"Failed fetching Yahoo data for {ticker} after 3 attempts")
    return pd.DataFrame()

# --- Load COT Data ---
def load_cot_data(asset_name: str) -> pd.DataFrame:
    logger.info(f"Fetching COT data for {asset_name}")
    where_clause = f'market_and_exchange_names="{asset_name}"'
    attempt = 0
    while attempt < 3:
        try:
            results = client.get(
            ...
            if not results:
                logger.warning(f"No COT data for {asset_name}")
                return pd.DataFrame()
            ...
            attempt += 1
            time.sleep(2)
    logger.error(f"Failed fetching COT data for {asset_name} after 3 attempts")
    return pd.DataFrame()
...
def preprocess_data(price_df: pd.DataFrame, cot_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cot_subset = cot_df[["report_date", "commercial_net", "non_commercial_net"]].rename(columns={"report_date": "Date"})
    df = pd.merge_asof(df.sort_values("Date"), cot_subset.sort_values("Date"), on="Date")
